...
Представляю исследовательский и программный пайплайн для систематической торговли на Московской бирже. Основные эксперименты и результаты в этой версии проекта получены на акциях; отдельная полноценная проверка на фьючерсах в итоговые результаты не включалась.
...
5. довести исследование до виртуальных заявок через брокерский API в тестовую среду, записывать все действия и ограничивать возможные убытки.
...
- `micro_dev` — отклонение microprice;
  >  **micro_dev ≈ (microprice - mid) / mid × 10 000**
...
Перед обучением и при разборе результатов отдельно анализировались распределения доходностей, просадки, скользящий коэффициент Шарпа, качество сигнала по временным периодам и причины ухудшения результата на новых данных. Для этого используются `src/analytics/diagnostics.py`, `scripts/38_alpha_diagnostics.py`, `scripts/45_forward_failure_diagnostics.py` и `scripts/70_regime_failure_analysis.py`.

Основной вывод EDA и последующей диагностики: краткосрочные зависимости в данных есть, но их величина и знак меняются между периодами. Поэтому одного среднего результата на всей истории недостаточно — дальше все гипотезы проверялись по времени и с учётом издержек.
...
- **Парный арбитраж (Pairs Trading)** — стратегия, в которой ищутся два инструмента с устойчивой статистической связью.
  ...
  Для отбора пар использовалась проверка на коинтеграцию — устойчивую долгосрочную статистическую связь между временными рядами. После этого рассчитывалось отклонение спреда между двумя инструментами от его обычного уровня. Если расхождение становилось достаточно большим, стратегия открывала позиции с ожиданием восстановления прежнего соотношения цен.
...
- walk-forward — несколько последовательных проверок на более поздних временных окнах;
- запрет пересекающихся сделок по одному тикеру;
- проверка результата без лучшего дня и без лучшего тикера;
- freeze параметров и отдельный forward-тест после фиксации стратегии;
- учёт комиссии и spread на входе и выходе;
- песочница для оценки расхождения между теоретическим и фактическим исполнением;
- для ранней RL-ветки — PBO/CSCV как дополнительная оценка риска переобучения backtest.
...
### 7.3. Triple Barrier и стоимость исполнения
...
### 7.4. Реальные сделки виртуальной песочницы
...
- песочница не полностью воспроизводит реальное биржевое исполнение заявок.
...
3. отдельное моделирование разных типов исполнения заявок;
