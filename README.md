...
Выпускной Data Science-проект: полный исследовательский и программный pipeline для систематической торговли на Московской бирже — от получения данных и построения признаков до walk-forward проверки, моделирования транзакционных издержек и исполнения в песочнице брокера.

> Работа носит исследовательский характер и не является инвестиционной рекомендацией. Реальные денежные средства в экспериментах не использовались; исполнение выполнялось в sandbox/теневом режиме.
...
3. валидировать их только по времени и отдельно оценивать out-of-sample/forward результат;
4. учитывать комиссию, spread и проскальзывание, а не только прогнозную метрику модели;
5. довести исследование до sandbox-исполнения с журналированием, риск-лимитами и мониторингом.

## 2. Почему эта задача сложнее обычного ML

В торговле высокая accuracy/AUC не гарантирует положительный PnL. Малый прогнозный edge может полностью исчезнуть после комиссии и spread. Кроме того, финансовые ряды нестационарны: стратегия, хорошо работающая на одном временном участке, может потерять эффект при смене рыночного режима.
...
- дневные и часовые OHLCV-свечи;
- поток сделок;
- 10 уровней стакана в реальном времени;
- агрегированные признаки микроструктуры;
- данные T-Bank Invest API для sandbox-исполнения;
- MOEX/AlgoPack-источники для исторических микроданных.

Полный локальный `data_cache/` занимает сотни мегабайт и не включён в Git. Для проверки формата в `data/sample/` лежат небольшие примеры свечей, trades и order book.

Код загрузки и сбора данных находится в `scripts/01_download_data.py`, `scripts/30_collect_orderbook.py`, `src/data/` и `src/lob/`.

## 4. Предобработка и признаки

Для свечных моделей формируются доходности, волатильность и факторные признаки. Для микроструктуры используются, в частности:

- `mid` — mid-price;
- `spread_bp` — spread в базисных пунктах;
- `ret_1m`, `ret_5m` — краткосрочная доходность;
- `OFI` — order flow imbalance;
- `TFI` — trade flow imbalance;
- `imb1`, `imb5` — дисбаланс стакана;
- `micro_dev` — отклонение microprice;
- market breadth / доля растущих инструментов.
